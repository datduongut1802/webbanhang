...
const CreateContainer = () => {
    ...
    const uploadImage = (e) => {
        ...
        uploadTask.on('state_changed', (snapshot) => {
            ...
            console.log(error);
            setFields(true);
            setMsg('Error while uploading : Try again <3');
    ...
    const deleteImage = () => {
        ...
        deleteObject(deleteRef).then(() => {
            setImageAsset(null);
            setIsLoading(false);
            setMsg('Image delete successfully <3');
            setAlertStatus('success');
            setTimeout(() => {
                setFields(false);
            }, 4000);
    ...
    const saveDetails = () => {
        ...
        try {
            if(!title || !calories || !imageAsset || !price || !category) {
                ...
                setIsLoading(false);
                setFields(true);
                setMsg('Data uploaded successfully <3');
                clearData();
                setAlertStatus("success");
                setTimeout(() => {
                    setFields(false);
            ...
            items-center justify-center gap-4">
                {
                    fields && (
                        <motion.p 
                            ...
                            animate={{ opacity : 1}}
                            exit={{ opacity : 0}}                      
                            className={`w-full p-2 rounded-lg text-center text-lg font-semibold ${  // String literals
                                alertStatus === 'danger' 
